...
@section('title', 'Nominee Review')

@section('content')
<div class="w-full">
    {{-- Header --}}
    <div class="mb-6 flex items-center gap-3">
        <a href="{{ route('admin.verification.nominee') }}" class="text-[var(--theme-primary)] hover:underline flex items-center gap-1 font-medium text-sm">
            <i class="ph ph-arrow-left"></i> Back
        </a>
        <h1 class="text-xl font-bold text-gray-800">Nominee Review</h1>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="p-5">
                <ul class="space-y-4 text-sm">
                    <li class="flex items-center justify-between">
                        ...
                        <span class="font-medium text-gray-900">{{ $nominee->user->name }}</span>
                    </li>
                    <li class="flex items-center justify-between">
                        <span class="text-gray-500">Username</span>
                        <a href="{{ route('admin.users.details', $nominee->user->username) }}" class="font-medium text-[var(--theme-primary)] hover:underline">{{ '@' . $nominee->user->username }}</a>
                    </li>
                    <li class="flex items-center justify-between">
                        <span class="text-gray-500">User Type</span>
                        <span class="font-medium text-gray-900">{{ ucfirst($nominee->user->account_type ?? 'Investor') }}</span>
                    </li>
                </ul>
            </div>
        </div>
        ...
        <div class="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden h-fit">
            <div class="p-5 border-b border-gray-100 flex items-center gap-2">
                <i class="ph ph-users text-[var(--theme-primary)] text-lg"></i>
                <h3 class="font-bold text-gray-800">Nominee Info</h3>
            </div>
            <div class="p-5">
                <ul class="space-y-4 text-sm mb-6">
                    <li class="flex items-center justify-between">
                        <span class="text-gray-500">Nominee Name</span>
                        <span class="font-medium text-gray-900">{{ $nominee->name }}</span>
                    </li>
                    <li class="flex items-center justify-between">
                        <span class="text-gray-500">Relation</span>
                        <span class="font-medium text-gray-900">{{ $nominee->relation }}</span>
                    </li>
                    <li class="flex items-center justify-between">
                        <span class="text-gray-500">Submitted</span>
                        <span class="font-medium text-gray-900">{{ $nominee->created_at->format('M d Y, H:i') }}</span>
                    </li>
                ...
                @if($nominee->status === 'pending')
                    <div class="flex flex-col gap-3">
                        <form action="{{ route('admin.verification.nominee.status', $nominee->id) }}" method="POST">
                            @csrf
                            ...
                            <button type="submit" class="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-[#00A843] text-white rounded-lg font-medium hover:bg-green-700 transition-colors">
                                <i class="ph ph-check-circle text-lg"></i> Approve Nominee
                            </button>
                        </form>
                        <form action="{{ route('admin.verification.nominee.status', $nominee->id) }}" method="POST">
                            @csrf
                            ...
                            <button type="button" onclick="const msg = prompt('Enter rejection reason (optional):'); if(msg !== null) { this.nextElementSibling.value = msg; this.closest('form').submit(); }" class="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-red-50 text-red-500 border border-red-200 rounded-lg font-medium hover:bg-red-100 transition-colors">
                                <i class="ph ph-x-circle text-lg"></i> Reject Nominee
                            </button>
                            <input type="hidden" name="admin_message" value="">
                        </form>
                    </div>
                @else
                    <div class="p-4 rounded-lg border {{ $nominee->status === 'approved' ? 'bg-green-50 border-green-100 text-green-700' : 'bg-red-50 border-red-100 text-red-700' }}">
                        <p class="font-medium text-sm flex items-center gap-2 mb-1">
                            <i class="ph {{ $nominee->status === 'approved' ? 'ph-check-circle' : 'ph-x-circle' }}"></i> 
                            Nominee is already {{ $nominee->status }}.
                        </p>
                        @if($nominee->admin_message)
                            <p class="text-xs opacity-90 mt-1">Reason: {{ $nominee->admin_message }}</p>
                        @endif
                    </div>
                @endif
            </div>
            ...
            <div class="p-5">
                <div class="space-y-6">
                    <div>
                        ...
                        @if($nominee->identity_front_proof)
                            <a href="{{ Storage::url($nominee->identity_front_proof) }}" target="_blank" class="block rounded-lg overflow-hidden border border-gray-200 hover:border-[var(--theme-primary)] transition-colors">
                                <img src="{{ Storage::url($nominee->identity_front_proof) }}" alt="Front Side" class="w-full object-cover max-h-48">
                            </a>
                        @else
                    ...
                    <div>
                        ...
                        @if($nominee->identity_back_proof)
                            <a href="{{ Storage::url($nominee->identity_back_proof) }}" target="_blank" class="block rounded-lg overflow-hidden border border-gray-200 hover:border-[var(--theme-primary)] transition-colors">
                                <img src="{{ Storage::url($nominee->identity_back_proof) }}" alt="Back Side" class="w-full object-cover max-h-48">
                            </a>
                        @else
                            <div class="p-4 bg-gray-50 border border-gray-100 rounded-lg text-center text-gray-400 text-sm">
                                No image provided
