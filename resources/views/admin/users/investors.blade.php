...
<div class="min-h-full p-4 sm:p-6">
    <div class="bg-white rounded-none shadow-sm overflow-hidden">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-4 border-b border-gray-100">
            <h1 class="text-base font-semibold text-gray-700">All Investors</h1>
                <form method="GET" action="{{ route('admin.users.investors') }}" class="flex items-center gap-2 border border-gray-200 rounded-none px-3 py-1.5 w-full sm:w-64 focus-within:border-[var(--theme-primary)] transition-colors">
                    <input
                        type="text"
                        name="search"
                        value="{{ $search }}"
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-4 py-3.5 text-xs space-y-0.5">
                            <p><span class="font-medium text-gray-600">Type:</span> <span class="{{ $user->account_type == 'Root Distributor' ? 'text-purple-600 font-semibold' : 'text-gray-500' }}">{{ $user->account_type === 'Normal User' ? 'Investor' : $user->account_type }}</span></p>
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="text-center py-12 text-gray-400 text-sm">No investors found</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
