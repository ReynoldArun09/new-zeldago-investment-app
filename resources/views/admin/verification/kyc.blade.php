...
@section('title', 'All KYC Submissions')
...
<div class="w-full">
    ...
    <div class="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
        ...
        <div class="px-6 py-5 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <h2 class="text-lg font-bold text-gray-800">All KYC Submissions</h2>
            <form action="{{ route('admin.verification.kyc') }}" method="GET" class="flex">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Name / Doc No." class="w-full md:w-64 px-4 py-2 text-sm border border-gray-200 rounded-l-lg focus:outline-none focus:border-[var(--theme-primary)]">
            ...
            <table class="w-full text-left text-sm text-white">
                <thead class="bg-[var(--theme-primary)] font-medium">
                    <tr>
                        ...
                        <th class="px-6 py-3.5">Document</th>
                ...
                <tbody class="text-gray-600 divide-y divide-gray-100">
                    @forelse($kycs as $kyc)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="font-medium text-gray-900">{{ $kyc->user->name }}</div>
                                <a href="{{ route('admin.users.details', $kyc->user->username) }}" class="text-xs text-[var(--theme-primary)] hover:underline">{{ '@' . $kyc->user->username }}</a>
                            </td>
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border border-blue-200 text-blue-600 bg-white">
                                    {{ ucfirst($kyc->user->account_type ?? 'Investor') }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="font-medium text-gray-700">{{ $kyc->document_type }}</div>
                                <div class="text-xs text-gray-400">{{ $kyc->document_number ?? '---' }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <a href="{{ route('admin.verification.kyc.review', $kyc->id) }}" class="inline-flex items-center justify-center px-3 py-1.5 border border-indigo-200 rounded-md text-xs font-medium text-[var(--theme-primary)] bg-white hover:bg-indigo-50 focus:outline-none transition-colors">
                                    Review
                                </a>
                            </td>
                        </tr>
        ...
        @if($kycs->hasPages())
            <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
                <span class="text-sm text-gray-500">
                    Showing {{ $kycs->firstItem() ?? 0 }} to {{ $kycs->lastItem() ?? 0 }} of {{ $kycs->total() }} results
                </span>
                <div>
                    {{ $kycs->appends(request()->query())->links('pagination::tailwind') }}
                </div>
            </div>
        @else
            <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
                <span class="text-sm text-gray-500">
                    Showing {{ $kycs->count() }} to {{ $kycs->count() }} of {{ $kycs->count() }} results
                </span>
