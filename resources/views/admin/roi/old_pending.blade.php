...
@section('title', 'Pending ROI Requests (Old)')
...
<div class="w-full">
    ...
    <div class="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
        ...
        <div class="px-6 py-5 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <h2 class="text-lg font-bold text-gray-800">Pending ROI Requests (Old)</h2>
            <form action="{{ route('admin.old-roi.pending') }}" method="GET" class="flex">
                ...
                <tbody class="text-gray-600 divide-y divide-gray-100">
                    @forelse($logs as $log)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2" x-data="{ openReject: false }">
                                    <form action="{{ route('admin.roi.process', $log->id) }}" method="POST">
                                        @csrf
